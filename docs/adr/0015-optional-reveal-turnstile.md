...
- **Reveal-Turnstile is a per-clip, creator-set flag, default off.** The create form carries `requireTurnstile` (a DO column `require_turnstile`); when unset, revealing needs no human. This makes the default poof revealable by anyone who holds the link, including a headless agent, the CLI, the MCP server, or a raw API caller.
...
- **Issue #18 is resolved for non-gated poofs:** the CLI/MCP/core can reveal a PIN poof with just the PIN. A `requireTurnstile` poof remains browser-only by design.
